function [res, Z_align] = ablation_Z(fea, gt, dim_c, r1Temp, anchor_num, W, A, Z)
...
%     rho = min(rho*eta, max_rho);
    ...
    flag = 0;
    for p = 1 : view_num
%         if (norm(W{p}*X{p}-A{p}*Z{p}, inf) > 1e-7)
%             obj1(iter) = norm(W{p}*X{p}-A{p}*Z{p}, inf);
%             flag = 1;
%         end
